    ...
    <div className="p-6 max-w-3xl mx-auto">
          ...
          <div>
            ...
            {invoice.due_date && (
              <p className="text-xs text-gray-400 mt-0.5">
                Vence: {new Date(invoice.due_date).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-6">
        ...
        <div className="flex justify-between flex-wrap gap-4">
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">De</p>
          ...
          {invoice.clients && (
            <div className="text-right">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Para</p>
              <p className="text-sm font-semibold text-gray-900">{invoice.clients.first_name} {invoice.clients.last_name}</p>
        ...
        <div className="flex gap-8 text-sm border-t border-gray-50 pt-4">
          <div>
            ...
            <p className="font-medium text-gray-900 mt-0.5">
              {new Date(invoice.issue_date).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          </div>
          {invoice.due_date && (
            <div>
              ...
              <p className="font-medium text-gray-900 mt-0.5">
                {new Date(invoice.due_date).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="border-t border-gray-50 pt-4">
          <div className="grid grid-cols-[1fr_60px_80px_90px] text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2 px-1">
            <span>Concepto</span><span className="text-center">Cant.</span><span className="text-right">Precio</span><span className="text-right">Total</span>
          </div>
        ...
        <div className="flex flex-col items-end gap-1.5 border-t border-gray-50 pt-4">
          ...
          {invoice.tax_rate > 0 && (
            <div className="flex gap-12 text-sm">
              <span className="text-gray-500">Impuesto ({invoice.tax_rate}%)</span>
