...
import {
  ...
  XCircle, FileText, Users, ArrowRight,
} from 'lucide-react';
...
export default function TrabajosPage() {
  ...
  const filtered = jobs.filter(j => {
    ...
    const matchSearch = [
      j.title, j.clients?.first_name, j.clients?.last_name,
  ...
  const counts = TABS.reduce((acc, t) => {
    acc[t.key] = t.key === 'all' ? jobs.length : jobs.filter(j => j.status === t.key).length;
    return acc;
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <Link href="/dashboard/trabajos/nuevo">
          <Button size="md"><Plus size={15} className="mr-1.5"/> Nuevo trabajo</Button>
        </Link>
      </div>

      {/* Search + Tabs */}
      <div className="flex flex-col gap-3 mb-5">
        <Input placeholder="Buscar por nombre, cliente, ciudad..." value={search}
            ...
            return (
              <div key={job.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
                ...
                {job.status !== 'invoiced' && job.status !== 'cancelled' && (
                  <div className="border-t border-gray-50 px-5 py-2.5 flex items-center gap-2 overflow-x-auto">
