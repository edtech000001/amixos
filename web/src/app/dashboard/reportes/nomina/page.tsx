...
export default function NominaPage() {
  ...
  useEffect(() => {
    ...
    // Walk back period by period until the start matches (history is past).
  ...
  }, [periodParam, periodApplied, business, frequency, anchorDate]);
  ...
  const onDeleteLoan = async (id: string) => {
    if (!business) return;
    const ok = await confirmDialog({ message: t.dashboard.reports.payroll.loanDeleteConfirm, destructive: true });
    if (!ok) return;
    setBusy(true);
