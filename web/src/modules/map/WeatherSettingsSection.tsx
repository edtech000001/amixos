...
export function WeatherSettingsSection({ config, onChange }: Props) {
  ...
  return (
    <div>
      <p className="text-xs font-semibold text-gray-400 uppercase mb-1">{t.sectionTitle}</p>
      <p className="text-xs text-gray-500 mb-3">{t.sectionSubtitle}</p>

      <div className="bg-gray-50 rounded-2xl p-4 space-y-4">
        {/* Enabled */}
        <div className="flex items-center justify-between gap-3">
          <div className="flex-1">
            <p className="text-sm font-semibold text-gray-900">{t.enabledLabel}</p>
            <p className="text-xs text-gray-500 mt-0.5">{t.enabledSubtitle}</p>
          </div>
          <button
            type="button"
            onClick={() => onChange({ ...config, enabled: !config.enabled })}
            className={`relative w-11 h-6 rounded-full transition-colors ${config.enabled ? 'bg-primary' : 'bg-gray-200'}`}
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-1">{t.retentionLabel}</label>
          <input
            type="number"
            min={1}
            max={90}
          ...
          />
          <p className="text-[10px] text-gray-400 mt-1">{t.retentionSubtitle}</p>
        </div>

        {/* Focus radius — used by the map's "Storm focus" toggle. */}
        <div>
          <label className="block text-xs text-gray-500 mb-1">{t.proximityRadiusLabel}</label>
          <input
            type="number"
            min={1}
            max={500}
            ...
            className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900"
          />
          <p className="text-[10px] text-gray-400 mt-1">{t.proximityRadiusSubtitle}</p>
        </div>

        {/* Excluded states — parsed on blur (see mobile section comment). */}
        <div>
          <label className="block text-xs text-gray-500 mb-1">{t.excludedStatesLabel}</label>
          <input
            type="text"
            value={excludedDraft}
        ...
        <div>
          ...
          >
            <div className="flex-1">
              <p className="text-xs font-semibold text-gray-700">
                {t.eventsHeading}{' '}
                <span className="text-gray-400 font-normal">({config.events.length})</span>
              </p>
              <p className="text-[10px] text-gray-400 mt-0.5">{t.eventsSubtitle}</p>
            </div>
            <ChevronDown
              size={16}
              className={`text-gray-500 shrink-0 transition-transform ${eventsExpanded ? 'rotate-180' : ''}`}
            />
          </button>
          ...
          <div className="space-y-2 mt-2">
            {config.events.length === 0 ? (
              <p className="text-xs text-gray-500 italic">{t.eventsEmpty}</p>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full shrink-0 transition-colors ${isEnabled ? 'bg-primary' : 'bg-gray-200'}`}
                      ...
                      <button
                        ...
                        className={`flex-1 flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-3 py-2 hover:bg-gray-50 text-left ${dimClass}`}
                      >
                        <span className={`flex-1 text-sm truncate ${ev.event ? 'text-gray-900' : 'text-gray-400'}`}>
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg hover:bg-gray-100 ${dimClass}`}
                        aria-label="Remove"
                      >
            ...
            <button
              type="button"
              onClick={addEvent}
              className="w-full flex items-center justify-center gap-1 py-2 rounded-xl border border-dashed border-gray-300 hover:bg-gray-100"
            >
...
function EventPicker({
  ...
  return (
    <Modal open onClose={onClose} title={t.eventPickerTitle} size="sm">
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-100 mb-3">
      ...
      <div className="max-h-[55vh] overflow-y-auto -mx-2 px-2">
        {isEmpty ? (
          <p className="text-xs text-gray-400 italic mt-4 text-center">{t.eventPickerNoResults}</p>
        ) : (
          filteredGroups.map((group) =>
            group.events.length === 0 ? null : (
              <div key={group.category} className="mt-2 first:mt-0">
                {!q && (
                  <p className="px-3 pt-2 pb-1 text-[10px] font-semibold uppercase text-gray-400">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left ${
                        isTaken ? 'opacity-40 cursor-not-allowed' : 'hover:bg-gray-100'
                      }`}
                    >
                      <span className={`text-sm ${isCurrent ? 'text-primary font-semibold' : 'text-gray-800'}`}>
                        {name}
                      </span>
