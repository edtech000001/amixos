-- 227 — Branch-scope the reports overview
--
-- The reports screen has a branch switcher, but reports_overview only ever
-- took a business id, so every figure stayed business-wide and the control did
-- nothing. p_location_id null = all branches (unchanged behaviour).
--
-- Scoped by the table's own location_id (invoices, jobs, inventory — see
-- migrations 104/158). Clients are deliberately NOT scoped: client_locations
-- is many-to-many with "no link = shared everywhere", so a per-branch client
-- count would either read zero for businesses that never linked a client, or
-- double-count shared ones. The per-branch figures worth having — revenue,
-- jobs, hours — all come from tables that do carry a branch.
--
-- Rows with a NULL location_id are excluded from a branch view: they belong to
-- no branch, and counting them everywhere would make the branch totals sum to
-- more than the business total.
--
-- Otherwise identical to the 207 version.
--
