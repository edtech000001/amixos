...
-- Strategy: Create ALL tables first, then apply ALL policies
...
create table if not exists public.clients (
  ...
  postal_code text,
  country text,
  loyalty_status text not null default 'good_standing',
  birthday date,
  notes text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
create policy "Members can view their own record"
  on public.business_members for select using (user_id = auth.uid());

-- business_modules
create policy "Business members can view modules"
  on public.business_modules for select using (
    exists (
      select 1 from public.business_members
...
create policy "Owners can manage modules"
  on public.business_modules for all using (
    exists (
      select 1 from public.business_members
      where business_members.business_id = business_modules.business_id
      and business_members.user_id = auth.uid()
...
create policy "Owners and managers can update timesheets"
