// The update prompt. Bottom-anchored (one-hand reach), dismissible, and it
// never acts on its own — see lib/updates/useAppUpdate.ts for why restarting
// is always a deliberate tap.
//
...
export function UpdateBanner() {
  ...
  const { otaReady, storeUpdate, suppressed, restart, dismiss } = useAppUpdate();
  ...
  return (
    ...
    >
      <View className="flex-row items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3 shadow-lg">
        ...
        <Pressable onPress={dismiss} hitSlop={8} className="p-1 active:opacity-60">
